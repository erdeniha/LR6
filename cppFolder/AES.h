...
void CipherBlock(uint8_t state[16], const std::vector<uint8_t>& roundKeys, bool printDebug = false);

std::vector<uint8_t> AddPadding(const std::string& input);
std::vector<uint8_t> RemovePadding(const std::vector<uint8_t>& input);

std::vector<uint8_t> CryptOFB(const std::vector<uint8_t>& input, const std::vector<uint8_t>& key, const std::vector<uint8_t>& iv, bool debugBlock = false);
